    ...
    <div class="form form_branch_in">
          ...
          <h2>
            {{
              Base.StateForm === ActionTable.Add
                ? $t("common.add_record", {
                    module: $t(`module.cash.${ModuleName.Branch}_small`),
                  })
      ...
      <div class="form-container">
        <div class="form-item_input">
          ...
          <div class="form-group ms-small">
            <base-input
              ...
              :maxLength="20"
              :messageValid="
                t('validate.empty', { field: t('module.cash.branchCode') })
              "
              :label="$t('common.id')"
          ...
          <div class="form-group ms-big">
            <base-input
              ...
              :maxLength="255"
              :messageValid="
                t('validate.empty', { field: t('module.cash.branchName') })
              "
              :label="$t('common.name')"
        ...
        <div class="form-group">
          ...
          <textarea class="input" v-model="branch.description"></textarea>
        </div>
      </div>
  ...
  try {
    ...
    if (messValid.length > 0) {
      ...
      props.Base.showNotificationError(messValid, () => {
        props.Base.focusInputError("form_branch_in"), listenEvent();
      });
    } else {
      if (
...
const handleCloseModal = () => {
  try {
    if (
      JSON.stringify(branch.value) != JSON.stringify(branchComparison.value)
    ) {
      unListenEvent();
