    ...
    <div class="form form_category_in">
      ...
      <div class="form-container">
        <div class="form-item_input">
          ...
          <div class="form-group ms-small">
            <base-input
              ...
              :messageValid="
                t('validate.empty', {
                  field: t('module.inventory.categoryCode'),
                })
              "
              :label="$t('common.id')"
          ...
          <div class="form-group ms-big">
            <base-input
              ...
              :messageValid="
                t('validate.empty', {
                  field: t('module.inventory.categoryName'),
                })
              "
              :label="$t('common.name')"
        ...
        <div class="form-group">
          ...
          <textarea class="input" v-model="category.description"></textarea>
        </div>
      </div>
  ...
  try {
    ...
    if (messValid.length > 0) {
      ...
      props.Base.showNotificationError(messValid, () => {
        props.Base.focusInputError("form_category_in"), listenEvent();
      });
    } else {
      if (
...
function validateInput() {
  return [
    /** Validate code */
    category.value.categoryCode.trim() === ""
      ? t("validate.empty", { field: t("module.inventory.categoryCode") })
      : null,
    /** Validate name */
    category.value.categoryName.trim() === ""
      ? t("validate.empty", { field: t("module.inventory.categoryName") })
      : null,
  ].filter((item) => {
    if (item) {
      return true;
...
const handleCloseModal = () => {
  try {
    if (
      JSON.stringify(category.value) != JSON.stringify(categoryComparison.value)
    ) {
      unListenEvent();
